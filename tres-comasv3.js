                ...
                try {
                    ...
                    for (let item of req.files) {
                        ...
                        if (el.engine == "aws-s3") {
                            ...
                            if (el.structure_folder == 'date') {
                                path_ = folder + moment().format('YYYY') + '/' + moment().format('MM') + '/' + moment().format('DD') + '/' + Key

                            } else if (el.structure_folder == 'extension') {
                                path_ = folder + ext.toUpperCase() + '/' + Key

                            } else if (el.structure_folder == 'custom') {
                                path_ = folder + el.custom_folder_name + '/' + Key

                            } else if (el.structure_folder == 'alphabetic') {
                                let vo = file.originalname.split('')
                                vo = vo[0]
                                path_ = folder + vo.toUpperCase() + '/' + Key
                            ...
                            let params = {
                                ...
                                ACL: el.connect.acl,
                                ContentDisposition: el.connect.contentDisposition
                            };


                            await el.s3Client.send(new PutObjectCommand(params));

                            let newfile = {
                                url: `https://${Bucket}.s3.${el.connect.region}.amazonaws.com/${Key}`,
                                name: originalname,
                                extension: mimetype,
                                filename: Key,
                                path: path_,
                                ...
                                token: token,
                                link: `https://${Bucket}.s3.${el.connect.region}.amazonaws.com/${Key}`
                    ...
                    for (let item of req.files) {
                        ...
                        if (el.engine == "aws-s3") {
                            ...
                            if (el.structure_folder == 'date') {
                                ...
                                path = folder + el.custom_folder_name + '/' + Key
